	...
	<head>
        ...
        <script type="application/javascript">
			function gotoUrl(xLink,xItc,DueDate,flag) {
				...
				window.location.href = xLink+"?xItc="+xItc+"&xDate="+DueDate+"&xUrl=m2_2.php"+"&flag="+flag;
			}
...
<? if($Fl==2){ ?>
    <fieldset data-role="controlgroup" data-mini="true">
    	<input type="radio" name="radio-mini" id="radio-mini-1" value="choice-1"  onClick='gotoFilter("m2_2.php","<?=$eDate?>",1,"<?=$flag?>");'/>
    	<label for="radio-mini-1">เรียงตามราคา</label>

		<input type="radio" name="radio-mini" id="radio-mini-2" value="choice-2"  checked="checked"onClick='gotoFilter("m2_2.php","<?=$eDate?>",2,"<?=$flag?>");'/>
    	<label for="radio-mini-2">เรียงตามห้อง/ราคา</label>
	</fieldset>
<? }else{ ?>
    <fieldset data-role="controlgroup" data-mini="true">
    	<input type="radio" name="radio-mini" id="radio-mini-1" value="choice-1" checked="checked" onClick='gotoFilter("m2_2.php","<?=$eDate?>",1,"<?=$flag?>");'/>
    	<label for="radio-mini-1">เรียงตามราคา</label>

		<input type="radio" name="radio-mini" id="radio-mini-2" value="choice-2"  onClick='gotoFilter("m2_2.php","<?=$eDate?>",2,"<?=$flag?>");'/>
    	<label for="radio-mini-2">เรียงตามห้อง/ราคา</label>
	</fieldset>
<? } ?>
...
WHERE
(
	( buffer_fac_order.CrQty + buffer_fac_order.RvQty ) < buffer_fac_order.SoQty
	OR
	( buffer_fac_order.CrQty + buffer_fac_order.RvQty ) > 0
)
