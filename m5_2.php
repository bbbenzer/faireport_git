...
<html>
	<head>
        ...
        <script type="application/javascript">
			function gotoUrl(xLink,xItc,DueDate,flag) {
				...
				window.location.href = xLink+"?xItc="+xItc+"&xDate="+DueDate+"&xUrl=m5_2.php"+"&flag="+flag;
			}
			...
			function gotoFilter(xLink,DueDate,Fl,IS,flag) {
	...
	<div data-role="content">
	  <h3>สินค้าที่ผลิตเกิน เฉพาะบรรจุ2</h3>

<? if($Fl==2){ ?>
    <fieldset data-role="controlgroup" data-mini="true">
    	<input type="radio" name="radio-mini" id="radio-mini-1" value="choice-1"  onClick='gotoFilter("m5_2.php","<?=$eDate?>",1,"<?=$flag?>");'/>
    	<label for="radio-mini-1">เรียงตามราคา</label>

		<input type="radio" name="radio-mini" id="radio-mini-2" value="choice-2"  checked="checked"onClick='gotoFilter("m5_2.php","<?=$eDate?>",2,"<?=$flag?>");'/>
    	<label for="radio-mini-2">เรียงตามห้อง/ราคา</label>
	</fieldset>
<? }else{ ?>
    <fieldset data-role="controlgroup" data-mini="true">
    	<input type="radio" name="radio-mini" id="radio-mini-1" value="choice-1" checked="checked" onClick='gotoFilter("m5_2.php","<?=$eDate?>",1,"<?=$flag?>");'/>
    	<label for="radio-mini-1">เรียงตามราคา</label>

		<input type="radio" name="radio-mini" id="radio-mini-2" value="choice-2"  onClick='gotoFilter("m5_2.php","<?=$eDate?>",2,"<?=$flag?>");'/>
    	<label for="radio-mini-2">เรียงตามห้อง/ราคา</label>
	</fieldset>
<? } ?>
...
WHERE buffer_fac_order.SoQty < buffer_fac_order.RvQty
	...
	$Sql .= "ORDER BY  buffer_fac_order.SalePrice ASC";
			...
			?>
				<tr>
					...
					<td width="200px" style="cursor: pointer;" onClick='gotoUrl("Sale.php","<?=$Result["Item_Code"]?>","<?=$eDate?>","<?=$flag?>")'><?=$Result["NameTH"]?></td>
                    ...
                    <td width="50px" style="cursor: pointer;" onClick='gotoUrl("Receive.php","<?=$Result["Item_Code"]?>","<?=$eDate?>","<?=$flag?>")'><?=$Result["RvQty"]?></th>
